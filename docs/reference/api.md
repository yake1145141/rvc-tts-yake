...
服务端是一个 FastAPI 应用，启动后自带交互式文档：

```text
http://<服务器IP>:8080/docs
```

（如果设了 `webui.password`，`/docs` 也需要先登录控制台。）

## 鉴权

`security.api_key` 非空时，业务接口必须带密钥，三种传法任选：
...
`webui.password` 非空时，管理类接口还需要先登录控制台（会话 Cookie），
详见[网页控制台](/guide/webui#哪些接口受保护)。
...
```json
{
  ...
  "engine": {
    ...
    "stats": {
      ...
      "cache_hit": 3, "last_success_at": "2026-09-18 10:10:00",
      "last_duration": 2.31
    }
  },
  "storage": {
...
```json
{
  ...
  "gpu": {
    ...
    "processes": [
      { "pid": 4498, "name": "/opt/tts-server/venv/bin/python", "used_memory_mb": 548.0 }
    ],
    "torch": {
  ...
  "system": {
    ...
    "disk": { "path": "/opt/tts-server/output", "total_gb": 117.6,
              "used_gb": 15.8, "used_pct": 13.4 },
    "uptime_seconds": 3600
  }
}
```
...
完整错误码表见[错误码速查](/guide/troubleshooting)。
